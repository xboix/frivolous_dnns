...
def plot_single_data(data):
    ...
    for idx_plot, num_data in enumerate(nuance_settings):
        ...
        means = layer_data['NN_ave'].mean()
        errs = layer_data['NN_std'].mean()
    ...
    #pyplot.ylim([-0.6, 7])
    ax.set_title(DATASET + ' - Neuron Similarity')
    ax.set_xlabel('Model Size Factor')
    ax.set_ylabel('Amount of Similar Neurons per Neuron')
...
def main():
    ...
    plot(data, plot_single_data,
         plotting_columns=[ 'not_selective','cross_validation','NN_ave','NN_std','training_dropout', 'training_weight_decay',
                            'training_data_augmentation', 'random_labels', 'non_zero', 'num_components',
                            'cross_validation', 'selectivity_mean','selectivity_std','selectivity_gen_mean','selectivity_gen_std']
         ...
         ignored_key_columns=['num_components'], results_subdirectory='NN')
