...
def plot_single_data(data, area_under_curve_func=np.trapz):
    ...
    for idx_plot, num_data in enumerate(nuance_settings):
        ...
        means.plot(yerr=errs, xticks=means.index, linewidth=ll_width[idx[idx_plot]],
                   label=name_settings[idx[idx_plot]],  linestyle=linestyles[idx[idx_plot]], color=cc[idx[idx_plot]], logx=True, ax=ax)
    ...
    ax.set_title(DATASET + ' \n Area Under the Ablation Curve ')
    ax.set_xlabel('Model Size Factor')
    ax.set_ylabel('AUC Ablation')
    #pyplot.ylim([0.15, 0.72])
    ...
    #pyplot.gcf().subplots_adjust(bottom=0.13)
    return fig
...
def main():
    ...
    data = data[data['perturbation_name'] == "Activation Knockout"].copy()
    data = data[data['perturbation_layer'] == "all"].copy()


    plot(data, plot_single_data,
         plotting_columns=['training_dropout', 'random_labels','training_weight_decay', 'training_data_augmentation', 'random_labels', 'condition', 'perturbation_amount', 'performance', 'cross_validation'] + \
                          [column for column in data.columns if column.startswith('multiplier_layer')],
         results_subdirectory='multiplier_robustness')
