...
def plot_single_data(data):
    ...
    for idx_plot, num_data in enumerate(nuance_settings):
        ...
        means = layer_data['selectivity_mean'].mean()
        errs = layer_data['selectivity_std'].mean()
    ...
    #pyplot.ylim([0, 1])
    ax.set_title('CIFAR-10 - Selectivity')
    ax.set_xlabel('Model Size Factor')
    ax.set_ylabel('Average Selectivity')
...
def main():
    ...
    plot(data, plot_single_data,
         ...
         ignored_key_columns=['num_components'], results_subdirectory='Selectivity')
