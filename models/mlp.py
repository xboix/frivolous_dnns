...
def MLP3(x, opt):
    ...
    with tf.name_scope('fc2') as scope:
        W = tf.Variable( tf.truncated_normal([int(32*opt.dnn.neuron_multiplier[0]), int(32*opt.dnn.neuron_multiplier[1])], dtype=tf.float32,
                                stddev=1e-3*opt.dnn.init_mult), name='weights')
        b = tf.Variable(0 * tf.ones([int(32*opt.dnn.neuron_multiplier[1])]), name='bias')

        fc2 = tf.nn.relu(tf.nn.bias_add(tf.matmul(fc1, W), b, name=scope))
        activations += [fc2]

    # fc3
    with tf.name_scope('fc2') as scope:
        W = tf.Variable( tf.truncated_normal([int(32*opt.dnn.neuron_multiplier[1]), int(32*opt.dnn.neuron_multiplier[2])], dtype=tf.float32,
                                stddev=1e-3*opt.dnn.init_mult), name='weights')
        b = tf.Variable(0 * tf.ones([int(32*opt.dnn.neuron_multiplier[2])]), name='bias')

        fc3 = tf.nn.relu(tf.nn.bias_add(tf.matmul(fc2, W), b, name=scope))
        activations += [fc3]


    # fc8
    with tf.name_scope('fc_out') as scope:
        W = tf.Variable(tf.truncated_normal([int(32*opt.dnn.neuron_multiplier[2]), 2],
                                            dtype=tf.float32, stddev=1e-2*opt.dnn.init_mult), name='weights')
        b = tf.Variable(tf.zeros([2]), name='bias')

        fc8 = tf.nn.bias_add(tf.matmul(fc3, W), b, name=scope)
        activations += [fc8]

    return fc8, [], activations


def MLP3_test(x, opt, select, labels_id, dropout_rate, perturbation_params, perturbation_type):
    ...
    with tf.name_scope('fc2') as scope:
        W = tf.Variable(
            tf.truncated_normal([int(32 * opt.dnn.neuron_multiplier[0]), int(32 * opt.dnn.neuron_multiplier[1])], dtype=tf.float32,
                                stddev=1e-3),
            name='weights')
        b = tf.Variable(0.1 * tf.ones([int(32 * opt.dnn.neuron_multiplier[1])]), name='bias')
        ...
        fc2_predrop = tf.nn.relu(tf.nn.bias_add(tf.matmul(dropout1, W), b, name=scope))

        if perturbation_type == 3:
            fc2_predrop = pt.activation_noise(fc2_predrop, perturbation_params[3][1], opt.hyper.batch_size)
        elif perturbation_type in [2, 4]:
            ss = tf.reshape(tf.tile(select[1], [opt.hyper.batch_size]), [-1, int(fc2_predrop.get_shape()[1])])
            fc2_predrop = pt.activation_knockout_mask(fc2_predrop, perturbation_params[4][1], ss)
    ...
    with tf.name_scope('fc2') as scope:
        W = tf.Variable(
            tf.truncated_normal([int(32 * opt.dnn.neuron_multiplier[1]), int(32 * opt.dnn.neuron_multiplier[2])], dtype=tf.float32,
                                stddev=1e-3),
            name='weights')
        b = tf.Variable(0.1 * tf.ones([int(32 * opt.dnn.neuron_multiplier[2])]), name='bias')
        ...
        fc3_predrop = tf.nn.relu(tf.nn.bias_add(tf.matmul(dropout2, W), b, name=scope))
        ...
        if perturbation_type == 3:
            ...
            ss = tf.reshape(tf.tile(select[0], [opt.hyper.batch_size]), [-1, int(fc3_predrop.get_shape()[1])])
    ...
    # fc8
    with tf.name_scope('fc_out') as scope:
        W = tf.Variable(tf.truncated_normal([int(32 * opt.dnn.neuron_multiplier[2]), len(labels_id)],
