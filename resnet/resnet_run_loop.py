...
def process_record_dataset(dataset,
    ...
    Args:
      ...
      augment: A boolean denoting whether to augment the training data
    ...
    dataset = dataset.map(
        lambda value: parse_record_fn(value, is_training, dtype, augment),  # *SC*
...
def resnet_model_fn(features, labels, mode, size_factor, model_class,
    ...
    Args:
      ...
      size_factor: a float giving the size factor
    ...
    model = model_class(resnet_size, size_factor, data_format, resnet_version=resnet_version,
                        dtype=dtype)  # *SC*

    # *SC*
    logits, activations = model(features, mode == tf.estimator.ModeKeys.TRAIN)
    # *SC*
...
def resnet_main(
    ...
    # *SC*
    model_dir = flags_obj.model_dir + experiments.opt[flags_obj.opt_id].name + '/'
    if os.path.isdir(model_dir):
        shutil.rmtree(model_dir)
    # *SC*

    classifier = tf.estimator.Estimator(
        model_fn=model_function, model_dir=model_dir,  # *SC*
        config=run_config,
    ...
    train_hooks = hooks_helper.get_train_hooks(
        flags_obj.hooks,
        model_dir=model_dir,  # *SC*
        batch_size=flags_obj.batch_size)

    def input_fn_train(num_epochs, input_context=None):
        return input_function(
            is_training=True,
            augment=experiments.opt[flags_obj.opt_id].hyper.augmentation,  # *SC*
    ...
    def input_fn_eval():
        return input_function(
            is_training=False,
            augment=False,  # experiments.opt[flags_obj.opt_id].hyper.augmentation,  # *SC*
...
def define_resnet_flags(resnet_size_choices=None, dynamic_loss_scale=False,
    ...
    choice_kwargs = dict(
        name='resnet_size', short_name='rs', default='56',
