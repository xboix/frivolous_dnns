...
for opt in run_opts:
    ...
    header = ['model_name', 'evaluation_set', 'layer', 'size_factor', 'batch_size', 'similarity_mean',
              'similarity_std', 'compressability_95', 'selectivity_mean', 'selectivity_std', 'nonselective',
              'top_1_acc', 'top_5_acc']

    if not os.path.isfile(opt.results_dir + opt.name + '/selectivity.pkl'):
    ...
    with open(opt.csv_dir + opt.name + '_redundancy.csv', 'w') as csvfile:
        ...
        if os.path.isfile(opt.results_dir + opt.name + '/accuracy' + '.pkl'):
            with open(opt.results_dir + opt.name + '/accuracy' + '.pkl', 'rb') as f:
        ...
        if os.path.isfile(opt.results_dir + opt.name + '/similarity' + '.pkl'):
            with open(opt.results_dir + opt.name + '/similarity' + '.pkl', 'rb') as f:
        ...
        if os.path.isfile(opt.results_dir + opt.name + '/compressability' + '.pkl'):
            with open(opt.results_dir + opt.name + '/compressability' + '.pkl', 'rb') as f:
        ...
        if os.path.isfile(opt.results_dir + opt.name + '/selectivity' + '.pkl'):
